<template>
  <div class="tag-edit">
    <div class="edit-header">
      <h2>{{ isEdit ? '编辑标签' : '新建标签' }}</h2>
      <p class="subtitle">标签用于文章分类和检索</p>
    </div>
      ...
      <div class="form-section">
        ...
        <n-grid :cols="1" :x-gap="24" responsive="screen">
          <n-form-item label="标签名称" path="name">
            <n-input v-model:value="form.name" placeholder="标签名称" size="large" />
          </n-form-item>

          <n-form-item label="URL Slug" path="slug">
            <n-input v-model:value="form.slug" placeholder="tag-slug" />
          </n-form-item>
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      // 更新标签
      await tagApi.update(route.params.id as string, form.value)
